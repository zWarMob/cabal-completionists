...
$(document).on("click","input[type='checkbox']",(evt)=>{console.log(evt)});
...
function populateTabs(){
    ...
    for(tab in collection){
        ...
        tabString+=`<div id="${tabFix}" data-tab-name="${tab}" class="tab-link${isFirstTab?" active":""}">${tab}<input type="checkbox" data-tab="${tab}"></div>`
        tabContentString+=`<div id="${tabFix}-content" class="tab-content${isFirstTab?" active":""}"></div>`;
        isFirstTab = false;
    }
    $(".tabs").html(tabString);
...
function populateSubTabs(tab, tabFix){
    ...
    for(subTab in tabObj){
        var subTabFix = subTab.toLowerCase().replace("'","").replaceAll(" ","").replaceAll("(","").replaceAll(")","");
        subTabString+=`<div id="${subTabFix}" data-subtab-name="${subTab}" class="subtab-link${isFirstSubTab?" active":""}"><input type="checkbox" data-tab="${tab}" data-subtab="${subTab}">${subTab}</div>`
        subTabContentString+=`<div id="${subTabFix}-content" class="sub-tab-content${isFirstSubTab?" active":""}"></div>`
        isFirstSubTab = false;
    }
...
$(".subtab-link").click((evt)=>{
    ...
    for (i = 0; i < subtabcontents.length; i++) {
        subtabcontents[i].className = subtabcontents[i].className.replace(" active", "");
    }
...
function populateSubTab(subTab, subTabFix, tab){
    ...
    for(subTabItem in subTabObj){
        ...
        subTabContentItemsString+=`<div class="item">
            ...
            <div class="item-image"><img src="public/item-images/${subTabItemFolderFix}.png" /></div>
